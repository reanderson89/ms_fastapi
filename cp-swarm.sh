#!/usr/bin/env bash
#
# This is just a quick and dirty way to copy necerssary config files onto the development Docker Swarm cluster
#

declare -a HOSTS=(172.27.0.3 172.27.0.4 172.27.0.5)
USERNAME="blueboard"
DEST_PATH="/home/${USERNAME}"

# SSH keys
TMP_FILE=$(mktemp)
cat <<EOF >> ${TMP_FILE}
ecdsa-sha2-nistp256 AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBAbTWF7F4lq11r/YeiZETcA1+pLmhQwN4W8TQSr3F2XLdMTMNaSQcPIkehhLAvjIwD1+87B8CbeJvl4SsSj2uMY= Clark
ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIKo3tSAUxTAD0/IQIpQUe5sMjseuGN4gN6hIF+0pRPpM Ryan
ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIEf3z5CMjoKvEJcdIrBdDsbjMArxsgXCTODYnaYxyn3j Jason
ssh-rsa  Josh
ssh-rsa  Owen
ecdsa-sha2-nistp256 AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBPel3IUgtx0ZuC0UoVHCGOoKf5GhPKljzgG8Mrx4vOWCgcQPlgS8hKAvEYfR5AxhpGp4GCkVv+DqMBO3HsBUMDA= Github Action
EOF

for HOST in "${HOSTS[@]}"
do
    # copy the run script
    scp run-milestones-swarm.sh "${USERNAME}@${HOST}:${DEST_PATH}/run.sh"
    scp run-galera-swarm.sh "${USERNAME}@${HOST}:${DEST_PATH}/galera.sh"
    ssh -t ${USERNAME}@${HOST} "chmod 755 ${DEST_PATH}/*.sh"

    # OLD!  copy the swarm stack definition
    # scp docker-swarm.yml "${USERNAME}@${HOST}:${DEST_PATH}/docker-swarm.yml"
    # scp galera-swarm.yml "${USERNAME}@${HOST}:${DEST_PATH}/galera-swarm.yml"

    scp ${TMP_FILE} "${USERNAME}@${HOST}:~/.ssh/authorized_keys"
    
done

rm -f ${TMP_FILE}