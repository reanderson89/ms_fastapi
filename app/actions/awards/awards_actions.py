...
from app.models.award import AwardUpdate
...
class AwardActions:
    ...
    async def check_if_award_exists(name, error = True):
        award = await BaseCRUD.check_if_exists(AwardModelDB, [AwardModelDB.name == name])
        if award and error:
    ...
    async def delete_award(cls, award_uuid: str):
        ...
        """
        client_award = await BaseCRUD.check_if_one_exists(
            ClientAwardModelDB,
