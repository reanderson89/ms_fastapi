...
async def test_integration_get_budget():
	...
	async with httpx.AsyncClient(app=app, base_url="http://test") as ac:
		response = await ac.get(f"/clients/{client_uuid}/budgets/{budget_uuid}")
		assert response.status_code == 200
...
async def test_integration_update_budget():
	...
	budget_uuid = "test_budget_uuid"
	async with httpx.AsyncClient(app=app, base_url="http://test") as ac:
		response = await ac.put(f"/clients/{client_uuid}/budgets/{budget_uuid}")
		assert response.status_code == 200
...
async def test_integration_delete_budget():
	...
	budget_uuid = "test_budget_uuid"
	async with httpx.AsyncClient(app=app, base_url="http://test") as ac:
		response = await ac.delete(f"/clients/{client_uuid}/budgets/{budget_uuid}")
		assert response.status_code == 200
