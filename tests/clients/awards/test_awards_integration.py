...
async def test_integration_get_award():
	...
	award_9char = "testchr"
	async with httpx.AsyncClient(app=app, base_url="http://test") as ac:
		response = await ac.get(f"/clients/{client_uuid}/awards/{award_9char}")
		assert response.status_code == 200
...
async def test_integration_update_award():
	...
	award_9char = "testchr"
	async with httpx.AsyncClient(app=app, base_url="http://test") as ac:
		response = await ac.put(f"/clients/{client_uuid}/awards/{award_9char}")
		assert response.status_code == 200
...
async def test_integration_delete_award():
	...
	award_9char = "testchr"
	async with httpx.AsyncClient(app=app, base_url="http://test") as ac:
		response = await ac.delete(f"/clients/{client_uuid}/awards/{award_9char}")
		assert response.status_code == 200
