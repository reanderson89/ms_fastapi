...
def test_integration_create_message(program_message):
    assert "uuid" in program_message
    assert len(program_message["message_9char"]) == 9
    assert program_message["name"] == new_message["name"]
    assert program_message["body"] == new_message["body"]


def test_integration_get_messages(test_app, program_message):
...
def test_integration_get_message(test_app, program_message):
    ...
    assert response["uuid"] in program_message["uuid"]
...
def test_integration_update_message(test_app, program_message):
...
def test_integration_delete_message(test_app, program):
    try:
        message = test_app.post(f"/v1/clients/{program['client_uuid']}/programs/{program['program_9char']}/messages",  json=new_message).json()
