...
class Permissions:
    ...
    def __call__(self,
        ...
        if JWT_ENFORCED == "false":
            ...
            if self.level == "rails":
                try:
                    verify_rails_jwt = jwt.decode(auth.credentials, RAILS_JWT_SECRET_KEY, algorithms=[ALGORITHM])
                    if verify_rails_jwt:
                        return True
                except:
                    pass
...
def check_token(credentials):
    try:
        ...
        pass
    try:
        verify_rails_jwt = jwt.decode(credentials.credentials, RAILS_JWT_SECRET_KEY, algorithms=[ALGORITHM])
        if verify_rails_jwt:
            return True
        else:
            return False
